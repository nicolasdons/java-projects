...
double[] potenciasSimuladas = {100.0, 350.0, 800.0, 1500.0, 2400.0, 2900.0, 2700.0, 1800.0, 900.0, 200.0};
            ...
            for (int i = 0; i < potenciasSimuladas.length; i++) {
                ...
                double potencia = potenciasSimuladas[i]; // CORRIGIDO: Usa o valor do array
                double temperatura = 25.0 + (potencia / 100.0);

                Telemetria telemetria = new Telemetria();
                // Ajuste os nomes dos setters conforme a sua classe Telemetria.java (Lombok ou métodos manuais)
                telemetria.setEnergiagerada(potencia); 
                telemetria.setTemperatura(temperatura);
                telemetria.setDatahora(dataHora);
                telemetria.setPainel(p1); // Associa ao painel p1 para não vir null
